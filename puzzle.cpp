...
namespace puzzle_game {
    ...
    puzzle_piece* puzzle::ptr_piece_above(const puzzle_piece* piece){
        return ptr_piece_at(current_order_of_pieces,Vector2{piece->pos.x,piece->pos.y-piece->bounding_box.height});
    }

    puzzle_piece* puzzle::ptr_piece_below(const puzzle_piece* piece){
        return ptr_piece_at(current_order_of_pieces,Vector2{piece->pos.x,piece->pos.y+piece->bounding_box.height});
    }

    puzzle_piece* puzzle::ptr_piece_left(const puzzle_piece* piece){
        return ptr_piece_at(current_order_of_pieces,Vector2{piece->pos.x-piece->bounding_box.width,piece->pos.y});
    }

    puzzle_piece* puzzle::ptr_piece_right(const puzzle_piece* piece){
        return ptr_piece_at(current_order_of_pieces,Vector2{piece->pos.x+piece->bounding_box.width,piece->pos.y});
    }


    //function for grabbing a piece pointer
    puzzle_piece* puzzle::ptr_piece_at(std::vector<puzzle_piece>& list,const Vector2& pos){
        for(auto& p : list){
    ...
    std::vector<puzzle_piece*> puzzle::neighbours_of_last_piece(){
    ...
    void puzzle::handle_mouse_hover(const Vector2& mouse_pos){
        for(auto& p : current_order_of_pieces){
            if(CheckCollisionPointRec(mouse_pos,p.bounding_box)){
                Rectangle outline = {p.pos.x,p.pos.y,static_cast<float>(p.img.width),static_cast<float>(p.img.height)};
                DrawRectangleLinesEx(outline,constants::PUZZLE_PIECE_BORDER_THICKNESS,WHITE);
            }
        }
    ...
    void puzzle::draw(){
        ...
        for(auto& p : current_order_of_pieces){
            ...
            if(!is_filled_in_after_completion()){
                DrawRectangleLinesEx(Rectangle{p.pos.x,p.pos.y,static_cast<float>(p.img.width),static_cast<float>(p.img.height)},constants::PUZZLE_PIECE_BORDER_THICKNESS,BLACK);
            }
        }
    ...
    // std::vector<puzzle_piece>& puzzle::get_pieces(){
    //     return current_order_of_pieces;
    // }


    //used to scramble puzzle
    puzzle_piece* puzzle::ptr_rand_neighbour_of_last_piece(){
    ...
    void puzzle::set_image(Image* image,std::string file_path_for_title){
        ...
        util::setup_window(&modif_img,current_divisor);
        ...
        //util::set_window_icon(ImageCopy(*image));
    ...
    void puzzle::fill_list_with_pieces(std::vector<puzzle_piece>& list){
    ...
    void puzzle::increase_divisor(int add){
        current_divisor += add;
